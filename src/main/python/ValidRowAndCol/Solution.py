...
class Solution:
    def checkValid(self, matrix: List[List[int]]) -> bool:
        NUMS, TRANSPOSED = list(range(1, len(matrix[0]) + 1)), []
        for i in range(len(matrix[0])): TRANSPOSED.append([])
        for l in matrix:
            nums, idx = NUMS.copy(), 0
            for i in l:
                TRANSPOSED[idx].append(i)
                idx += 1 
...
print(*(row for row in zip(*failed)))
print(Solution().checkValid(failed))